...
def cluster_df_formation(master_data):
    
    cluster_df = pd.pivot_table(master_data,columns=['Year','Month'],index='Item_Code',values='Sales_Qty',fill_value=0,aggfunc=sum)

    run_repea_items = master_data[(master_data['Group_Labels']=='Runner_Items') | (master_data['Group_Labels']=='Repeater_Items')]['Item_Code'].unique()

    final_run_repea_items_list = []

    for i in run_repea_items:
        if cluster_df[cluster_df.index==i].values[0][-6:].mean() > 0:
            final_run_repea_items_list.append(i)
    
    return cluster_df,final_run_repea_items_list
...
def data_preperation(data_df,window_sizes,exp_window_sizes,lead_periods, lag_periods):
...
def STACKED_BIGRU_01(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
    ...
    model = Sequential()
    model.add(Bidirectional(GRU(264, activation='relu', input_shape=(n_lookback, multi_x_train.shape[2]), return_sequences=True)))
    ...
    model.fit(multi_x_train,multi_y_train,epochs=75,batch_size=4,verbose=False)
...
def STACKED_LSTM_01(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
    ...
    model = Sequential()
    model.add(LSTM(256, activation='relu', input_shape=(n_lookback, multi_x_train.shape[2]), return_sequences=True))
    model.add(Dropout(0.3))
    ...
    model.fit(multi_x_train,multi_y_train,epochs=80,batch_size=5,verbose=False)
...
def RandomForestRegressor_Model(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
...
def RandomForestRegressor_Model_01(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
    ...
    multi_x_train = multi_x_train.reshape(multi_x_train.shape[0],multi_x_train.shape[1]*multi_x_train.shape[2])
    multi_x_test = multi_x_test.reshape(multi_x_test.shape[0],multi_x_test.shape[1]*multi_x_test.shape[2])
    model = RandomForestRegressor( n_estimators=301,
    criterion='squared_error',
    max_depth=10,
    min_samples_split=5,
    min_samples_leaf=2)
...
def RandomForestRegressor_Model_02(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
    ...
    multi_x_train = multi_x_train.reshape(multi_x_train.shape[0],multi_x_train.shape[1]*multi_x_train.shape[2])
    multi_x_test = multi_x_test.reshape(multi_x_test.shape[0],multi_x_test.shape[1]*multi_x_test.shape[2])
    model = RandomForestRegressor( n_estimators=423,
    criterion='squared_error',
    max_depth=5,
    min_samples_split=3,
    min_samples_leaf=5)
...
def XGBOOST_03(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
    ...
    model = XGBRegressor(n_estimators=501,max_depth=7,learning_rate=0.01)
...
def NBEATS_01(cluster_df,item_code,multi_model_data):
    ...
    model = NBEATSModel(input_chunk_length=6, output_chunk_length=n_forecast,n_epochs =epoch_t

                        ,num_layers = 8,batch_size = 5,random_state=42)
...
def STACKED_GRU_03(cluster_df,multi_model_data,item_code,n_lookback,n_forecast,split_threshold):
    ...
    model.fit(multi_x_train,multi_y_train,epochs=100,batch_size=5,verbose=False)
